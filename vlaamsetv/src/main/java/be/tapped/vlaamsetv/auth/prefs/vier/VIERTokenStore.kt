...
class VIERTokenStoreImpl(context: Context, crypto: Crypto) : VIERTokenStore {
    ...
    private val vierTokenDataStore by lazy {
        context.createDataStore(fileName = "vier-token.pb", serializer = TokenSerializer(crypto))
    }

    private val credentialsDataStore by lazy {
        context.createDataStore(fileName = "vier-credentials.pb", serializer = VIERCredentialsSerializer(crypto))
    }
    ...
    override suspend fun vierCredentials(): Credential? = credentialsDataStore.data.firstOrNull()?.let {
    ...
    override suspend fun token(): TokenWrapper? = vierTokenDataStore.data.firstOrNull()?.let {
